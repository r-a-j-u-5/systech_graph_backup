...
dotenv.config();
  ...
  export const UpdateComponentIOT = async (req: Request, res: Response) => {
    try {
      try {
      ...
      for (let i = 0; i < data?.children?.length; i++) {
        let command = `g.V().hasId('${maincomponentid}').as('a').V().hasId('${data?.children[i]}').as('b').addE('manages').from('a').to('b')`;
        await axios
          .post(
            `${process.env.COMMAND_ENDPOINT}`,
            {
      ...
      for (let i = 0; i < data?.removechildren?.length; i++) {
        let command = `g.V().hasId('${maincomponentid}').outE('manages').where(inV().hasId('${data?.removechildren[i]}')).drop()
        `;
        await axios
          .post(
            `${process.env.COMMAND_ENDPOINT}`,
